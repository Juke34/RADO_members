...
dfr$fname <- NULL
dfr$img <- NULL
dfr <- dfr[,c("profile","name","category","workplace", "status","affiliation","city","keywords")]
```
...
col_worplace <- c('#017A4AFF','#3D98D3FF','#FF363CFF','#7559A2FF','#794924FF','#8CDB5EFF','#D6D6D6FF','#FB8C00FF')
...
if(length(col_worplace) < length(levels(factor(dfr$worplace)))) warning("Number of colours less than number of worplace.")
...
col_worplace <- col_worplace[1:length(levels(factor(dfr$worplace)))]
...
col_worplace2 <- col_worplace[factor(dfr$worplace)]
...
dfr$worplace <- paste0('<span class="color-tag" style="background-color:',col_worplace2,';">',dfr$worplace,'</span>')
```
...
<b>`r format(Sys.Date(),format="%Y")`</b> • SouthGreen
</span>
